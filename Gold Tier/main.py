...
def show_status():
    ...
    print(f"  Queued    : {count_files(queued)} files")
...
def update_dashboard():
    ...
    if count_files(queued):
        attention_items.append(f"- {count_files(queued)} queued action(s) waiting for service recovery")
...
- **Queued**: {count_files(queued)} files
...
def start_ralph_loop(*, once: bool = False, interval_seconds: int = 30):
    """Start the Gold Tier Ralph loop controller."""
...
Commands:
  ...
  --run-ralph-loop Start Gold Tier Ralph loop only
  --create-approval --action-type ... --objective ... --details ... [--mcp-args JSON]
    ...
    parser.add_argument("--run-ralph-loop", action="store_true", help="Run Gold Tier Ralph loop only")
    parser.add_argument("--ralph-once", action="store_true", help="Run one Ralph loop pass and exit")
    parser.add_argument(
        "--ralph-interval",
        type=int,
        default=30,
        help="Polling interval in seconds for Ralph loop (default: 30)",
    )
    ...
    elif args.run_ralph_loop:
        print("Starting Gold Tier Ralph loop...")
        start_ralph_loop(once=args.ralph_once, interval_seconds=args.ralph_interval)
